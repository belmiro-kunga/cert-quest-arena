...
const CookiePolicyPage = () => {
  const { cookiePolicy, loading, error } = usePolicies();
  const [lastUpdated, setLastUpdated] = useState<string>('17 de maio de 2025');

  useEffect(() => {
    if (cookiePolicy?.lastUpdated) {
      try {
        const date = new Date(cookiePolicy.lastUpdated);
          ...
          <CardContent className="prose prose-sm max-w-none">
            {loading ? (
              <div className="flex justify-center items-center py-12">
                ...
                <span className="ml-2">Carregando política de cookies...</span>
              </div>
            ) : error ? (
              <div className="text-red-500 py-4">
                <p>Ocorreu um erro ao carregar a política de cookies. Por favor, tente novamente mais tarde.</p>
              </div>
            ) : (
              <>
